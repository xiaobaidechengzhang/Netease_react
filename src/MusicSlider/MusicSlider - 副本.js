...
export default function MusicSlider(props) {
    ...
    const imgHover = (e, isHover, index) => {
        switch (index) {
            case 1:
                e.target.src = isPlay ? (isHover ? pauseActive : pauseNoActive) : (isHover ? playActive : playNoActive);
                break;
            case 2:
            ...
            case 6:
                ...
                if (ele6) {
                    //这样是为了保持图片处于hover状态
                    ele6.getElementsByClassName('musicImg')[0].src = !isMuted ? (isHover ? volumeActive : volumeNoActive) : (isHover ? mutedActive : mutedNoActive);
                }
                break;
            case 7:
                //获取父元素类为musicAcBtn的ele
                ...
                if (ele7) {
                    //这样是为了保持图片处于hover状态
                    ele7.getElementsByClassName('musicImg')[0].src = isHover ? listActive : listNoActive;
                }
                // e.target.src = isHover ? listActive : listNoActive;
                break;
            case 8:
                let hoverLoop = loopType == 1 ? noLoopActive : (loopType == 2 ? singleLoopActive : allLoopActive);
                let unHoverLoop = loopType == 1 ? noLoopNoActive : (loopType == 2 ? singleLoopNoActive : allLoopNoActive);
    ...
    const dropHandle = (e) => {
        ...
        dragEle.style.left = dragEle.style.left ? (parseInt(dragEle.style.left) + 50) + 'px' : '50px';
    ...
    const reset = () => {
        ...
        let bof = document.getElementById('bofang');
        let audio = document.getElementById('audio')
        bof.style.left = p;
        bb.style.width = p
        setSliper(p)
    ...
    useEffect(() => {
        ...
        back.addEventListener('click', (e) => {
            ...
            let btn = document.getElementById('bofang');
            let audio = document.getElementById('audio');
            let bb = document.getElementById('bb');
        ...
        bofang.addEventListener('click', (e) => {
        ...
        bofang.addEventListener('mouseup', (e) => {
            // startX = e.screenX;
            isMove = false;
        })
        ...
        document.addEventListener('click', (e) => {
            ...
            function isNameEle(tar, name1, name2) {
                ...
                if(tar.className) {
                    ...
                    if(tar.className.indexOf(name1) != -1 || tar.className.indexOf(name2) != -1) {
                        //如果是想要查找的元素, 返回true
                        return true
                    }
                }
        ...
        //为音量条添加滚轮滚动事件--改变音量大小
        let sliderCon = document.getElementsByClassName('sliderCon')[0];
        let audio = document.getElementById('audio')
        ...
        let backFast = document.getElementById('backFast');
        let fast = document.getElementById('fast');
        backFast.addEventListener('mousedown', () => {
            console.log('kasld;nflnadfna')
            outId = setTimeout(() => {
                id = setInterval(() => {
                    fastChange(1)
                }, 500)
    ...
    */
            ...
            >
                {
                    [1, 3, 5, 7, 6, 9].map((item, index) => {
                        return (
                            <MusicListItem key={index} index={index} dbClick={listItemDBClick}/>
                        )
                    })
                }
            </div>
    ...
    const changeColor = () => {
        let btn = document.getElementsByClassName('ant-btn')
        let i = 0;
        while(i<btn.length) {
            let mybtn = btn[i];
    ...
    return (
        <div>
            ...
            <div>
                ...
                <button onClick={seekTime}>
                    seek
                </button>
                <button onClick={upvolume}>
                    音量+
                </button>
                <button onClick={downvolume}>
                    音量-
                </button>
            ...
            <div style={{ margin: '0 50px', height: '10px', zIndex: 50 }} id='back'>
                <div style={{ height: '100%', backgroundColor: 'gray', opacity: 0.5, marginTop: 10, position: 'relative' }}>
                    <div
                        id='bb'
                        style={{ height: '100%', backgroundColor: 'blue', position: 'absolute', top: '50%', transform: 'translateY(-50%)', width: sliPer }}
                    >
                    </div>
                    <div
                        id='bofang'
                        style={{ width: '20px', height: '20px', borderRadius: '50%', backgroundColor: 'blue', position: 'absolute', top: '50%', transform: 'translate(-50%, -50%)', cursor: 'pointer', left: sliPer, boxShadow: '0 0 15px 5px blue' }}
                    >
            ...
            <div className='musicActions'>
                ...
                >
                    <img
                        className='musicImg'
                        src={loopType == 1 ? noLoopNoActive : (loopType == 2 ? singleLoopNoActive : allLoopNoActive)} style={{ width: 30, height: 30, objectFit: 'cover' }}
                    />
                </button>
                <div
            ...
            <Button onClick={changeColor}>第一个</Button>
            <Button>第二个</Button>
        </div>
    )
}